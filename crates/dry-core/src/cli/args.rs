...
pub struct Args {
    ...
    /// `dry-rs.toml`, falling back to the compiled-in default
    ...
    /// merger then consults `[output] format` from `dry-rs.toml`,
    ...
    /// Path to an explicit `dry-rs.toml` config file (bypasses auto-
    /// discovery). When set, the path MUST exist — missing path
    /// produces `ConfigError::Io` at startup. When unset, the loader
    /// auto-discovers a `dry-rs.toml` by walking upward from the
...
impl Args {
    ...
    pub fn from_matches(matches: &clap::ArgMatches) -> Result<Self, clap::Error> {
        ...
        // consult [gate]/[output] from dry-rs.toml" (per ADR D3).
