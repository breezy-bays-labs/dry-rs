...
use crate::domain::NormalizedTree;
// The anti-unification POD result types live in `crate::domain::template`
// (so `domain::Match` can carry a `Template` without `domain` depending
// on `comparison`). Re-export them here so existing consumers that name
// them through the engine — `comparison::antiunify::Template`, and the
// `pub use antiunify::{…}` block in `comparison/mod.rs` — keep resolving
// unchanged. This module owns the LGG *algorithm*; `domain` owns the
// *data*.
