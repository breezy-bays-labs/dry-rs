...
/// (`dry.example.toml`, dry-rs#77), the default extension allowlist
/// (`["rs"]`), and the documentation URIs.
///
...
pub const DRY4RS_META: AdapterMeta = AdapterMeta {
    ...
    long_about: "dry-rs detects structural duplication via per-subform fingerprinting + Jaccard \
                  ...
                  annotated example config. Universal flags `--top`/`--only-failing` reshape \
                  the displayed `view.*` projection; `result.*` stays unaffected per the \
                  truthful-gate ADR.",
    after_help: "",
