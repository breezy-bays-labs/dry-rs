...
//!
//! Asserts that dry-rs's workspace-root `dry.toml` is found by
...
//! - The literal `"dry.toml"` IS the point of this test
...
fn workspace_root_dry4rs_toml_discovered_and_parses() {
    ...
    let found = discover_config(workspace_root, "dry.toml")
        .expect("discover_config should not error on filesystem walk");

    let path = found.expect("dry.toml should exist at workspace root");
    assert_eq!(
        path.file_name().and_then(|n| n.to_str()),
        Some("dry.toml"),
        "discovered path filename is dry.toml"
    );

    let _config = load_config(&path).expect("dogfood dry.toml parses cleanly");
}
