...
dry4rs auto-discovers a `dry-rs.toml` config file by walking
...
regardless of what `dry-rs.toml` says. With no `--threshold` flag,
`[gate] threshold = 0.9` from `dry-rs.toml` applies; if neither
